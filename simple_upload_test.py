#!/usr/bin/env python3
"""上传 newapi-tools 到 Linux 测试机并运行测试"""
import paramiko
import os
import tempfile
...
def main():
    ...
    print(f"上传完成: {count}/{len(files)}", flush=True)

    # 运行单元测试
    print("\n运行单元测试...", flush=True)
    stdin, stdout, stderr = ssh.exec_command(
        f'cd {REMOTE_DIR}/test && timeout 120 bash unit-test.sh 2>&1'
    )
