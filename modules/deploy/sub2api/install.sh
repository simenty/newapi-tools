#!/bin/bash
# Sub2API 部署脚本 v2.2
# 支持 Sub2API (weishaw/sub2api)
set -eo pipefail

# 守卫：如果是被 source，直接 return（不执行安装逻辑）
...
_on_install_exit() {
    ...
    unset SESSION_SECRET API_KEY 2>/dev/null || true
}
...
log_info "=== 开始部署 Sub2API v2.2 ==="

# ---------- 显示新手提示 ----------
novice_prompt "即将部署 Sub2API。Sub2API 是一个 API 订阅转换平台。"

# ---------- 检查部署状态 ----------
if is_step_completed "sub2api_install"; then
    ui_warn "检测到已完成 Sub2API 部署"
...
# 使用智能默认值
SESSION_SECRET=$(get_config 'deploy.sub2api.session_secret')
if [[ -z "$SESSION_SECRET" ]]; then
    ui_info "正在生成会话密钥..."
    SESSION_SECRET=$(generate_password 32)
    set_config "deploy.sub2api.session_secret" "$SESSION_SECRET"
fi
SUB2API_PORT=$(get_config "deploy.sub2api.port" "8080")
...
if_expert && {
    ...
    if ask_yn "是否修改配置？" "n"; then
        read -r -p "Sub2API 端口 [${SUB2API_PORT}]: " input_port
        SUB2API_PORT=${input_port:-$SUB2API_PORT}
        set_config "deploy.sub2api.port" "$SUB2API_PORT"
    fi
}
...
# 从配置读取端口
SUB2API_PORT=$(get_config "deploy.sub2api.port" "8080")
NPM_HTTP_PORT=$(get_config "deploy.npm.port_http" "80")
NPM_HTTPS_PORT=$(get_config "deploy.npm.port_https" "443")
...
services:
  sub2api:
    ...
    restart: always
    ports:
      - "127.0.0.1:${SUB2API_PORT}:${SUB2API_PORT}"
    volumes:
      - ./data:/app/data
    environment:
      - SESSION_SECRET=${SESSION_SECRET}
    ...
    healthcheck:
      test: ["CMD", "wget", "-q", "--spider", "http://localhost:${SUB2API_PORT}"]
      interval: 10s
      timeout: 5s
      retries: 3
...
if [[ -f docker-compose.yml ]]; then
    ...
    # 兜底：检查默认镜像
    for img in weishaw/sub2api:latest jc21/nginx-proxy-manager:latest; do
        if ! docker image inspect "$img" &>/dev/null; then
            images_need_pull=1
            missing_images+=("$img")
        fi
...
mark_step_completed "sub2api_install"
set_state "sub2api.installed" "true"
set_state "sub2api.version" "latest"
set_state "sub2api.compose_file" "$NEWAPI_HOME/docker-compose.yml"
set_state "sub2api.env_file" "$NEWAPI_HOME/.env"

# ---------- 输出结果 ----------
echo ""
ui_success "=== Sub2API 部署完成 ==="
echo ""
echo -e "${UI_BOLD}访问地址：${UI_PLAIN}"
echo "  Sub2API 地址   : http://127.0.0.1:${SUB2API_PORT} （仅本机访问，需通过 NPM 反代暴露）"
echo "  NPM 管理地址  : http://$(hostname -I | awk '{print $1}'):${NPM_ADMIN_PORT}"
echo ""
ui_warn "安全提醒：请部署完成后配置 Sub2API 的访问凭据！"
echo ""
echo -e "${UI_BOLD}下一步：${UI_PLAIN}"
echo "  1. 在菜单中选择『3) 配置 SSL 与反向代理』来配置域名访问"
echo "  2. 查看 Sub2API 文档了解如何使用"

# 显示部署摘要
show_summary "部署摘要" \
    "✓ Sub2API 已启动（端口 ${SUB2API_PORT}）" \
    "✓ NPM 已部署（管理端口 ${NPM_ADMIN_PORT}）" \
    "✓ 所有服务健康状态正常"

# 发送 Webhook 通知
send_webhook "Sub2API 部署成功" "服务器 $(hostname) 的 Sub2API 已成功部署并启动"
