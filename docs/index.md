...
- **国际化**：`--lang en/zh-CN` 切换界面语言
- **审计日志**：所有命令自动记录到 `~/.config/newapi-tools/audit.log`
- **安全加固**：配置文件权限检查，敏感信息自动脱敏
- **交互式安装**：`install --interactive` 向导式配置端口/数据库/Redis
- **实时监控**：`status --watch` 实时刷新，`--all` 显示 CPU/MEM 统计
